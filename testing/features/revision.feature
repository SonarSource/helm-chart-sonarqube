...
功能: 支持扫描不同的 Git Revision
    ...
    场景: 分支扫描
        假定 执行 "sonar 扫描" 脚本成功
            ...
            | bash scripts/wait-sonar-analysis.sh '<config.{{.sonar.url}}>' '<config.{{.sonar.token}}>' revision-branch abc  |
        当 发送 "获取扫描结果" 请求
            """
    ...
    场景: PR 扫描
        假定 执行 "main 分支扫描" 脚本成功
            ...
            | bash scripts/wait-sonar-analysis.sh '<config.{{.sonar.url}}>' '<config.{{.sonar.token}}>' revision-pr main -Dsonar.host.url=<config.{{.sonar.url}}> -Dsonar.login=<config.{{.sonar.token}}> |
        并且 执行 "PR 源分支扫描" 脚本成功
            | command                                                                                                                                                                |
            ...
            | bash scripts/wait-sonar-analysis.sh '<config.{{.sonar.url}}>' '<config.{{.sonar.token}}>' revision-pr abc                                                              |
        当 发送 "获取扫描结果" 请求
            """
