...
- **Rotating `agenticSigningSecret` is a hard cutover today, not a rolling rotation.** `derive-keys.sh` supports emitting a `.previous` sibling key from a `<secret-file>.previous` instance secret, and the consumer pods accept one — the orchestrator verifies both renewal signatures and capability tokens against the current *and* previous generation for exactly this reason — but the instance secret is projected into the hook as a single file and the hook's per-consumer Secret projection is single-key, so a `.previous` key can never actually be produced or mounted. Rotating the instance secret and upgrading replaces every derived key at once; old and new pods briefly disagree on signatures during the rollout, and cross-component calls between them fail until the rollout completes. `orchestrator-job-capability` rotates with the rest, so the capability tokens held by jobs dispatched before the rotation stop verifying and their locator renewals fail — drain RUNNING jobs before rotating, same as for an orchestrator upgrade.

| Parameter                                       | Description                                                                                                     | Default                                                                |
| ------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------- |
| `vortexAnalysis.enabled`                        | Deploy Vortex and set `sonar.vortex.analysis.url` on the app nodes                          | `false`                                                                |
...
| `remediationAgent.enabled`                                       | Deploy the Remediation Agent (requires `agentOrchestrator.enabled=true` and `vortexAnalysis.enabled=true`)                                    | `false`                                                                        |
