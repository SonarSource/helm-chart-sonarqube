...
- **Ports must be paired.** Anything added to `agentEgressProxy.extraSafePorts`/`extraSslPorts` must also be added to `agentEgressProxy.networkPolicy.egressPorts`, or `NetworkPolicy` blocks the request before Squid's ACLs ever see it.
...
| `agentEgressProxy.image.digest`                                  | Pinned Squid image digest; blank falls back to `repository:tag`                                                                          | `""`                                                                           |
...
| `agentEgressProxy.nodeSelector`/`.tolerations`/`.affinity`       | Proxy pod scheduling                                                                                                                      | `{}` / `[]` / `{}`                                                             |
| `agentEgressProxy.resources`                                     | Proxy container resources                                                                                                                 | `{}`                                                                           |
...
| `agentEgressProxy.allowedDomains`                                | Domains runtimes may reach through the proxy (Squid `dstdomain` ACL); a leading dot allows the domain and all subdomains                  | `[]`                                                                           |
| `agentEgressProxy.extraSafePorts`/`.extraSslPorts`               | Extra ports beyond Squid's defaults (80 plain HTTP, 443 CONNECT/HTTPS); must also be added to `networkPolicy.egressPorts`                 | `[]`                                                                           |
| `agentEgressProxy.requestBodyMaxSizeMb`                          | Squid `request_body_max_size`, in MB                                                                                                     | `10`                                                                           |
| `agentEgressProxy.extraSquidConf`                                | Raw `squid.conf` lines appended after the generated ACLs                                                                                  | `""`                                                                           |
