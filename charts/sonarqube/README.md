...
| `<hunterAgent\|remediationAgent>.storage.pathStyle`               | Path-style addressing for `endpoint` above; omitted, not defaulted, so unset stays distinguishable from an explicit override once EA-926 merges this block over `agentOrchestrator.storage`  | unset (defers to `agentOrchestrator.storage.pathStyle`)                        |
