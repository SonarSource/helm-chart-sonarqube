...
| `prometheusExporter.podMonitor.enabled` | Enable Prometheus PodMonitor | `false` |
| `prometheusExporter.podMonitor.namespace` | Specify a custom namespace where the PodMonitor will be created | `""` |
| `prometheusExporter.podMonitor.interval` | Specify the interval how often metrics should be scraped | `30s` |
| `prometheusExporter.podMonitor.scrapeTimeout` | Specify the timeout after a scrape is ended | `""` |
| `prometheusExporter.podMonitor.jobLabel` |  Name of the label on target services that prometheus uses as job name | `""` |
