...
The Agent Orchestrator also needs a database: unless `jdbcOverwrite.enabled=true` (which it derives `agentOrchestrator.coreDb.endpoint`/`.name` from), you must set both explicitly — otherwise the install fails at template time rather than have the orchestrator silently inherit the `jdbcOverwrite.jdbcUrl` placeholder.
...
| `agentOrchestrator.coreDb.endpoint`                                   | CORE DB `host:port` (`CORE_DB_READ_WRITE_ENDPOINT`); blank = derived from `jdbcOverwrite.jdbcUrl` when `jdbcOverwrite.enabled=true`, otherwise required        | `""`                                                                           |
| `agentOrchestrator.coreDb.name`                                       | CORE DB name (`CORE_DB_NAME`); blank = derived from `jdbcOverwrite.jdbcUrl` when `jdbcOverwrite.enabled=true`, otherwise required          | `""`                                                                           |
