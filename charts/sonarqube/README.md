...
- **`"auto"` needs a live cluster, and fails open without one - but only that one way.** It reads the Service through Helm's `lookup`, which returns nothing under `helm template` or when the Service genuinely doesn't exist yet - so if you render manifests and apply them separately (ArgoCD and friends), the runtimes keep their kube-dns rule. The install warns when that happens; pin the address explicitly to close it. An identity that lacks `get`/`list` on Services in `istio.namespace` is a different failure mode, not this one: Helm's `lookup` only swallows a true not-found, so an RBAC-Forbidden error there aborts the entire `helm install`/`template` render instead of falling back to anything. Grant that RBAC, or pin an explicit address (or `""`), if it can't be granted.
