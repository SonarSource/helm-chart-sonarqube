...
**Scheduling.** Vortex, the Agent Orchestrator, and the Hunter/Remediation Agent runtimes each support their own `nodeSelector`/`affinity`/`tolerations`, which win over the chart's global `.Values.nodeSelector`/`.affinity`/`.tolerations` when set — same convention as `agentKeyDerivation` below. The chart-wide `priorityClassName` value applies to all of these pods automatically; there is no per-component `priorityClassName`.
...
- **Scheduling.** `agentKeyDerivation.nodeSelector`/`.tolerations`/`.affinity` place the hook Job's pod, same convention as the other agentic workloads — each falls back to the chart's global `.Values.nodeSelector`/`.tolerations`/`.affinity` when unset. The chart-wide `priorityClassName` applies to this pod too.
