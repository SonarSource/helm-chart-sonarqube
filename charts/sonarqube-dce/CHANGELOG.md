...
* Sandbox the agent runtimes with Kata Containers instead of gVisor when `OpenShift.enabled` is `true`, via the new `OpenShift.agentRuntimeClassName` (default `kata`). gVisor cannot work there — the installer DaemonSet needs containerd plus privileged/hostPID, which no default OpenShift SCC allows, and CRI-O ships no `runsc` handler — so every `gvisor.*` setting is ignored on OpenShift and no RuntimeClass or installer is rendered. The `RuntimeClass` itself is not created by the chart: it comes from the OpenShift sandboxed containers operator (`kata` for a default `KataConfig`, `kata-remote` for peer pods on AWS/Azure/vSphere), and the runtime pods stay unschedulable until it exists. Set `OpenShift.agentRuntimeClassName` to any other `RuntimeClass` you provisioned yourself, or to `""` to run the runtimes under the cluster's default runtime with no sandbox
