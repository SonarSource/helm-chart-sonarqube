...
* Raise the default `timeoutSeconds` to `5` on the `searchNodes` and `applicationNodes` readiness/liveness probes, so the forked `sh`/`curl` exec probe is not killed by the kubelet under CPU contention, and raise `applicationNodes.livenessProbe.failureThreshold` to `8` (restart window 240s, matching readiness)
