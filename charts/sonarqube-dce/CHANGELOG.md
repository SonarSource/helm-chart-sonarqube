...
* Point `agentOrchestrator`/`hunterAgent`/`remediationAgent` liveness probes at the dedicated `/livez` endpoint instead of the aggregate `/health`, and add `agentOrchestrator.terminationGracePeriodSeconds` (default `1320`)
