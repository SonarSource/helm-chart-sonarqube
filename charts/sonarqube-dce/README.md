...
The optional SonarQube Unified Agentic Harness — an Agent Orchestrator plus one Deployment/Service per enabled runtime family (`hunter`, `remediation`) — enabled via `agenticHarness.enabled`. When enabled, the chart also points the SonarQube app nodes at the orchestrator (via the `SONAR_HUNTERAGENT_ORCHESTRATOR_URL` / `SONAR_REMEDIATIONAGENT_ORCHESTRATOR_URL` env vars). `agenticHarness.agenticAnalysis.url` is independent of `agenticHarness.enabled` — it just tells SonarQube where to poll the (separately-deployed) Agentic Analysis service for health.
...
| `agenticHarness.agenticAnalysis.url`                             | Base URL of the (independently-deployed) Agentic Analysis / "Vortex" service; sets `SONAR_VORTEX_ANALYSIS_URL` (`sonar.vortex.analysis.url`) so SonarQube polls `<url>/health`, same as MCP health. Blank disables health polling | `""`                                                                           |
