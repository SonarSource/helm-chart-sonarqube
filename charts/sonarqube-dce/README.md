...
| `vortexAnalysis.nodeSelector`                   | Node labels for the Vortex analysis pod                                                                           | `{}`                                                                   |
...
| `orchestrator.serviceAccount.create`                             | Create a dedicated ServiceAccount for the orchestrator pod, independent of the top-level `serviceAccount`                                | `false`                                                                        |
| `orchestrator.serviceAccount.name`                               | Name of that ServiceAccount; defaults to `<fullname>-agentic-orchestrator` when `create` is true                                         | `""`                                                                           |
| `orchestrator.serviceAccount.automountToken`                     | Automount the ServiceAccount token into the pod; needed for IRSA                                                                          | `false`                                                                        |
| `orchestrator.serviceAccount.annotations`                        | Annotations for that ServiceAccount (e.g. an IRSA role ARN)                                                                              | `{}`                                                                           |
...
| `<hunterAgent\|remediationAgent>.serviceAccount.name`            | Name of that ServiceAccount; defaults to `<fullname>-agentic-runtime-<family>` when `create` is true                                     | `""`                                                                           |
...
| `orchestrator.image.repository`                                  | Agent Orchestrator image repository (required when enabled)                                                                              | `""`                                                                           |
| `orchestrator.image.tag`                                         | Agent Orchestrator image tag                                                                                                             | `""`                                                                           |
| `orchestrator.image.pullPolicy`                                  | Agent Orchestrator image pull policy                                                                                                     | `IfNotPresent`                                                                 |
| `orchestrator.image.pullSecrets`                                 | imagePullSecrets for the orchestrator image                                                                                              | `nil`                                                                          |
| `orchestrator.port`                                              | Port the orchestrator listens on (also set as `SERVER_PORT`)                                                                             | `8080`                                                                         |
| `orchestrator.replicaCount`                                      | Orchestrator replica count                                                                                                               | `1`                                                                            |
| `orchestrator.revisionHistoryLimit`                              | How many old ReplicaSets to retain; blank uses the Kubernetes default (`10`)                                                              | `""`                                                                           |
| `orchestrator.strategy`                                          | Deployment update strategy; blank uses the Kubernetes default (`RollingUpdate`)                                                           | `{}`                                                                           |
| `orchestrator.podLabels`                                         | Extra labels for the orchestrator pod                                                                                                     | `{}`                                                                           |
| `orchestrator.annotations`                                       | Extra annotations for the orchestrator pod                                                                                                | `{}`                                                                           |
| `orchestrator.nodeSelector`                                      | Orchestrator nodeSelector                                                                                                                 | `{}`                                                                           |
| `orchestrator.tolerations`                                       | Orchestrator tolerations                                                                                                                  | `[]`                                                                           |
| `orchestrator.affinity`                                          | Orchestrator affinity                                                                                                                     | `{}`                                                                           |
| `orchestrator.resources`                                         | Orchestrator container resources (cpu / memory / ephemeral-storage)                                                                       | requests `250m` / `512Mi` / `512Mi`, limits `1` / `1Gi` / `2Gi`                |
| `orchestrator.securityContext`                                   | Orchestrator pod security context                                                                                                        | `{}`                                                                           |
| `orchestrator.containerSecurityContext`                          | Orchestrator container security context, incl. `readOnlyRootFilesystem`                                                                  | [Restricted podSecurityStandard](#kubernetes---pod-security-standards)        |
| `orchestrator.probes.readiness`/`.liveness`                      | Orchestrator readiness/liveness probes (`enabled`, `path`, `initialDelaySeconds`, `periodSeconds`, `timeoutSeconds`, `successThreshold`, `failureThreshold`) | `enabled: true`, `/health`, see `values.yaml`          |
| `orchestrator.coreDb.endpoint`                                   | CORE DB `host:port` (`CORE_DB_READ_WRITE_ENDPOINT`); blank = derived from `jdbcOverwrite.jdbcUrl`                                        | `""`                                                                           |
| `orchestrator.coreDb.name`                                       | CORE DB name (`CORE_DB_NAME`); blank = derived from `jdbcOverwrite.jdbcUrl`                                                              | `""`                                                                           |
| `orchestrator.coreDb.username`                                   | CORE DB user (`CORE_DB_USERNAME`); blank = `jdbcOverwrite.jdbcUsername`                                                                  | `""`                                                                           |
| `orchestrator.coreDb.passwordSecretName`                         | Secret holding the CORE DB password (`CORE_DB_PASSWORD`); blank = the `jdbcOverwrite`-derived secret. Point at a raw-password secret if SonarQube's is encrypted/unavailable | `""`                                                                           |
| `orchestrator.coreDb.passwordSecretKey`                          | Key within `coreDb.passwordSecretName`; blank = the `jdbcOverwrite`-derived key                                                          | `""`                                                                           |
| `orchestrator.storage.endpoint`                                  | Object-storage endpoint; blank = real AWS S3 regional endpoint                                                                           | `""`                                                                           |
| `orchestrator.storage.region`                                    | Object-storage region                                                                                                                    | `us-east-1`                                                                    |
| `orchestrator.storage.bucket`                                    | Object-storage bucket for the shared job artifacts (required when enabled)                                                               | `agentic-jobs`                                                                 |
| `orchestrator.storage.pathStyle`                                 | Path-style addressing (`true` for MinIO, `false` for AWS S3)                                                                             | `true`                                                                         |
| `orchestrator.storage.accessKey`                                 | Inline S3 access key; leave blank for credential-less access (pod IAM role / IRSA)                                                       | `""`                                                                           |
| `orchestrator.storage.secretKey`                                 | Inline S3 secret key; leave blank for credential-less access                                                                             | `""`                                                                           |
| `orchestrator.storage.existingSecret`                            | Existing secret providing `AGENTIC_STORAGE_ACCESS_KEY` / `AGENTIC_STORAGE_SECRET_KEY`                                                     | `""`                                                                           |
| `orchestrator.sonarqubeToken.token`                              | Inline SonarQube token for the orchestrator (`AGENTIC_SONARQUBE_TOKEN`)                                                                   | `""`                                                                           |
| `orchestrator.sonarqubeToken.existingSecret`                     | Existing secret providing the SonarQube token                                                                                            | `""`                                                                           |
| `orchestrator.sonarqubeToken.existingSecretKey`                  | Key within `existingSecret` (defaults to `AGENTIC_SONARQUBE_TOKEN`)                                                                       | `""`                                                                           |
| `orchestrator.github.apiBaseUrl`                                 | `AGENTIC_GITHUB_API_BASE_URL`; blank = the orchestrator's default (real api.github.com)                                                  | `""`                                                                           |
...
| `<hunterAgent\|remediationAgent>.egressAllow`                    | Extra egress peers for the agent NetworkPolicy — each `{cidr}` or `{podSelector[,namespaceSelector]}`, with optional `ports`             | `[]`                                                                           |
...
Each agent also reads/writes job artifacts directly against `orchestrator.storage` via presigned URLs, bypassing the orchestrator. With that agent's `networkPolicy.enabled`, add a peer covering it to its `egressAllow`, or jobs fail at the first artifact download.
