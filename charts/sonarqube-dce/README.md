...
| `orchestrator.egressAllow`                                       | Extra egress peers for the orchestrator NetworkPolicy — each `{cidr}` or `{podSelector[,namespaceSelector]}`, with optional `ports`      | `[]`                                                                           |
...
| `<hunterAgent\|remediationAgent>.egressAllow`                    | Extra egress peers for the agent NetworkPolicy (same shape as the orchestrator's)                                                        | `[]`                                                                           |
...
Each agent also reads/writes job artifacts directly against `orchestrator.storage` via presigned URLs, bypassing the orchestrator. With that agent's `networkPolicy.enabled` (or the top-level `networkPolicy.enabled`), add a peer covering it to its `egressAllow`, or jobs fail at the first artifact download.
