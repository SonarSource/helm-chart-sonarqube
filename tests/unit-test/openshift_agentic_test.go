...
// and the Agent Egress Proxy's. All but the agent runtime ones carry a DNS egress rule from the
// shared helper unconditionally; the agent runtime policies only gain one once the pod actually
// has an Envoy to make the lookup pointless to withhold (see sonarqube.agentRuntime.hasEnvoy) -
// with istio.enabled defaulting to false, that fixture leaves them with none at all.
		...
		t.Run(chart.name, func(t *testing.T) {
			...
			for _, policy := range policies {
				t.Run(policy.Name, func(t *testing.T) {
					...
					if isAgentRuntimeNetworkPolicy(policy) {
						// istio.enabled defaults to false, so these pods get no Envoy and
						// correctly withhold DNS entirely (see sonarqube.agentRuntime.hasEnvoy).
						assert.Empty(t, rules, "no Envoy means no DNS egress rule to withhold in the first place")
						return
					}
...
// isAgentRuntimeNetworkPolicy identifies the hunter/remediation NetworkPolicies emitted by
// agent-networkpolicy.yaml, the only ones whose DNS egress rule is conditional on the pod
// actually getting an Envoy rather than being unconditional.
func isAgentRuntimeNetworkPolicy(policy networkingv1.NetworkPolicy) bool {
	return policy.Labels["sonarqube.agent/component"] == "runtime"
}
		...
		t.Run(chart.name, func(t *testing.T) {
			...
			for _, policy := range policies {
				t.Run(policy.Name, func(t *testing.T) {
					rules := dnsEgressRules(policy)
					if isAgentRuntimeNetworkPolicy(policy) {
						// Same reasoning as TestOpenShiftDnsEgressRule: istio.enabled defaults to
						// false regardless of OpenShift.enabled, so no Envoy, no DNS rule.
						assert.Empty(t, rules, "no Envoy means no DNS egress rule to withhold in the first place")
						return
					}
