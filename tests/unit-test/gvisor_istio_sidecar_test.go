...
const gvisorIstioSidecarValuesDir = "test-cases-values/sonarqube"

func renderGvisorIstioSidecar(t *testing.T, fixture string, templates []string, setValues map[string]string) (string, error) {
	t.Helper()
	opts := &helm.Options{
		Logger:      logger.Discard,
		ValuesFiles: []string{gvisorIstioSidecarValuesDir + "/" + fixture},
		SetValues:   setValues,
	}
	return helm.RenderTemplateE(t, opts, chartPath, releaseName, templates)
}
...
func gvisorIstioRuntimeDeployment(t *testing.T, fixture, family string, setValues map[string]string) appsv1.Deployment {
	t.Helper()
	output, err := renderGvisorIstioSidecar(t, fixture, []string{"templates/agent-runtime.yaml"}, setValues)
...
func gvisorIstioRuntimeService(t *testing.T, fixture, family string) corev1.Service {
	t.Helper()
	output, err := renderGvisorIstioSidecar(t, fixture, []string{"templates/agent-runtime-service.yaml"}, nil)
	require.NoError(t, err)
	for _, doc := range splitGvisorIstioDocs(output) {
...
func gvisorIstioRuntimeSidecar(t *testing.T, fixture, family string) (istioSidecar, bool) {
	t.Helper()
	...
	// not a test failure.
	output, err := renderGvisorIstioSidecar(t, fixture, []string{"templates/agent-runtime-sidecar.yaml"}, nil)
	if err != nil {
...
func gvisorIstioPeerAuthentications(t *testing.T, fixture string, setValues map[string]string) []istioPeerAuthentication {
	t.Helper()
	output, err := renderGvisorIstioSidecar(t, fixture, []string{"templates/peerauthentication.yaml"}, setValues)
	require.NoError(t, err)
...
func gvisorIstioNetworkPolicy(t *testing.T, fixture, family string) networkingv1.NetworkPolicy {
	t.Helper()
	output, err := renderGvisorIstioSidecar(t, fixture, []string{"templates/agent-networkpolicy.yaml"}, nil)
	require.NoError(t, err)
	for _, doc := range splitGvisorIstioDocs(output) {
...
func TestGvisorIstioSidecarContainer(t *testing.T) {
	for _, family := range []string{"hunter", "remediation"} {
		t.Run(family, func(t *testing.T) {
			deployment := gvisorIstioRuntimeDeployment(t, "gvisor-istio-sidecar.yaml", family, nil)
			...
			assert.Equal(t, "/health", probers["/app-health/agent-runtime/livez"].HTTPGet.Path)
...
func TestGvisorIstioSidecarResource(t *testing.T) {
	for _, family := range []string{"hunter", "remediation"} {
		t.Run(family, func(t *testing.T) {
			sidecar, found := gvisorIstioRuntimeSidecar(t, "gvisor-istio-sidecar.yaml", family)
			...
			assert.Equal(t, "sonarqube-agent-runtime-"+family, sidecar.Spec.WorkloadSelector.Labels["app"])
...
func TestGvisorIstioSidecarServiceTargetPort(t *testing.T) {
	for _, family := range []string{"hunter", "remediation"} {
		t.Run(family, func(t *testing.T) {
			service := gvisorIstioRuntimeService(t, "gvisor-istio-sidecar.yaml", family)
...
func TestGvisorIstioSidecarPeerAuthentication(t *testing.T) {
	pas := gvisorIstioPeerAuthentications(t, "gvisor-istio-sidecar.yaml", nil)

	byName := map[string]istioPeerAuthentication{}
	for _, pa := range pas {
		byName[pa.Metadata.Name] = pa
	}

	for _, suffix := range []string{"agent-runtime-hunter", "agent-runtime-remediation"} {
		var match *istioPeerAuthentication
		for name, pa := range byName {
	...
	var egressProxyPA *istioPeerAuthentication
	for name, pa := range byName {
		if strings.HasSuffix(name, "agent-egress-proxy") {
			pa := pa
			egressProxyPA = &pa
		}
	}
...
func TestGvisorIstioSidecarNetworkPolicy(t *testing.T) {
	for _, family := range []string{"hunter", "remediation"} {
		t.Run(family, func(t *testing.T) {
			policy := gvisorIstioNetworkPolicy(t, "gvisor-istio-sidecar.yaml", family)
			...
			var reachesIstiod bool
			for _, rule := range policy.Spec.Egress {
				for _, p := range rule.Ports {
					if p.Port.IntVal == 15012 {
						reachesIstiod = true
					}
				}
			}
			assert.True(t, reachesIstiod, "expected an egress rule reaching istiod on 15012")
		})
	}
}

// istio.gvisorSidecar.enabled=false must restore today's behaviour exactly: no init container, no
...
func TestGvisorIstioSidecarOffPath(t *testing.T) {
	deployment := gvisorIstioRuntimeDeployment(t, "gvisor-istio-sidecar-off.yaml", "hunter", nil)
	...
	service := gvisorIstioRuntimeService(t, "gvisor-istio-sidecar-off.yaml", "hunter")
	assert.Equal(t, "http", service.Spec.Ports[0].TargetPort.StrVal)

	_, found := gvisorIstioRuntimeSidecar(t, "gvisor-istio-sidecar-off.yaml", "hunter")
	assert.False(t, found, "no Sidecar should render when the feature is off")

	pas := gvisorIstioPeerAuthentications(t, "gvisor-istio-sidecar-off.yaml", nil)
	var egressProxyPA *istioPeerAuthentication
	var sawRuntimePA bool
	for _, pa := range pas {
		pa := pa
		if strings.HasSuffix(pa.Metadata.Name, "agent-egress-proxy") {
			egressProxyPA = &pa
		}
...
func TestGvisorIstioSidecarPortConflict(t *testing.T) {
	_, err := renderGvisorIstioSidecar(t, "gvisor-istio-sidecar-port-conflict.yaml", []string{"templates/agent-runtime.yaml"}, nil)
	require.Error(t, err)
	assert.Contains(t, err.Error(), "istio.gvisorSidecar.meshPort (8090) must differ from the hunter runtime port")
}

// The feature must stay off unless all three of istio.enabled, gvisor.enabled and
// istio.gvisorSidecar.enabled are true - any one missing must reproduce today's behaviour.
func TestGvisorIstioSidecarGatingMatrix(t *testing.T) {
	cases := []struct {
		...
		{"istio.gvisorSidecar.enabled=false", map[string]string{"istio.gvisorSidecar.enabled": "false"}},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			deployment := gvisorIstioRuntimeDeployment(t, "gvisor-istio-sidecar.yaml", "hunter", tc.setValues)
			assert.Empty(t, deployment.Spec.Template.Spec.InitContainers)
			assert.NotContains(t, deployment.Spec.Template.Labels, "security.istio.io/tlsMode")
		})
	}
