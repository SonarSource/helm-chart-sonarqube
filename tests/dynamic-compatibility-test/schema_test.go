...
func TestInvalidSchema(t *testing.T) {
	table := []struct {
	...
	}{
		{
			...
			valuesFilesPaths: []string{"sonarqube/invalid-edition.yaml"},
			expectedError:    "edition: edition must be one of the following: \"developer\", \"enterprise\"",
		},
		{
			testCaseName:     "no-default-edition",
			valuesFilesPaths: []string{"sonarqube/invalid-no-default-edition.yaml"},
			expectedError:    "edition is required",
		},
		{
			testCaseName:     "community-disabled-no-edition",
			valuesFilesPaths: []string{"sonarqube/invalid-community-disabled-no-edition.yaml"},
			expectedError:    "edition is required",
		},
	}
...
func TestNoTagLatestCommunity(t *testing.T) {
	helmOptions.ValuesFiles = []string{"sonarqube/test-notag-latest-cb.yaml"}
...
func TestNoTagAppVersionDeveloper(t *testing.T) {
	helmOptions.ValuesFiles = []string{"sonarqube/test-notag-appVersion-developer.yaml"}
	output, err := helm.RenderTemplateE(t, helmOptions, chartPath, releaseName, sqStsTemplate)
	assert.NoError(t, err)
...
func TestShouldUseBuildNumber(t *testing.T) {
	helmOptions.ValuesFiles = []string{"sonarqube/test-build-number.yaml"}
	output, err := helm.RenderTemplateE(t, helmOptions, chartPath, releaseName, sqStsTemplate)
	assert.NoError(t, err)
...
func TestShouldUseImageTag(t *testing.T) {
	helmOptions.ValuesFiles = []string{"sonarqube/test-image-tag-developer.yaml"}
	output, err := helm.RenderTemplateE(t, helmOptions, chartPath, releaseName, sqStsTemplate)
	assert.NoError(t, err)
	...
	expectedContainerImage := "sonarqube:2025.1.0-enterprise"
	actualContainers := renderedTemplate.Spec.Template.Spec.Containers
	assert.Equal(t, 1, len(actualContainers))
	assert.Equal(t, expectedContainerImage, actualContainers[0].Image)
}
...
func TestCustomCommunityTag(t *testing.T) {
	helmOptions.ValuesFiles = []string{"sonarqube/test-custom-image-values.yaml"}
	output, err := helm.RenderTemplateE(t, helmOptions, chartPath, releaseName, sqStsTemplate)
	assert.NoError(t, err)
...
func TestCommunityBuildNumberEmptyTag(t *testing.T) {
	helmOptions.ValuesFiles = []string{"sonarqube/test-community-build-empty-tag.yaml"}
	output, err := helm.RenderTemplateE(t, helmOptions, chartPath, releaseName, sqStsTemplate)
	assert.NoError(t, err)
...
func TestCommunityEmptyBuildNumberEmptyTag(t *testing.T) {
	helmOptions.ValuesFiles = []string{"sonarqube/test-community-empty-build-empty-tag.yaml"}
	output, err := helm.RenderTemplateE(t, helmOptions, chartPath, releaseName, sqStsTemplate)
	assert.NoError(t, err)
